	...
	(function() {
		...
		var _RAD2DEG = 180 / Math.PI,
			...
			bezierThrough = function(values, curviness, quadratic, basic, correlate, prepend) {
				...
				//check to see if the last and first values are identical (well, within 0.05). If so, make seamless by appending the second element to the very end of the values array and the 2nd-to-last element to the very beginning (we'll remove those capturePoints later)
				if (values.length > 1) {
			...
			_parseLengthData = function(obj, resolution) {
				...
				var a = [],
					...
					segments = [],
					curLS = [], //current length capturePoints array
					p, i, l, index;
				for (p in obj) {
				...
				return {length:total, lengths:lengths, capturePoints:segments};
			},
			...
			BezierPlugin = window._gsDefine.plugin({
					...
					init: function(target, vars, tween) {
						...
						if (this._timeRes) {
							...
							this._segments = ld.capturePoints;
